...
class Transformer_chanel(Module):
    def __init__(self,
                 d_model: int,
                 d_input: int,
                 d_channel: int,
                 ...
                 pe: bool = False,
                 mask: bool = False):
        super(Transformer_chanel, self).__init__()
    ...
    def forward(self, x, stage):
        ...
        # channel-wise
        # score矩阵为channel 默认不加mask和pe
        encoding_2 = self.embedding_input(x)
        channel_to_gather = encoding_2
...
class DGTN_RE(nn.Module):
    def __init__(self, block, layers, input_channels=12, inplanes=64, num_classes=9, device=''):
        ...
        # self.GTN = Transformer(d_model=512, d_input=235, d_channel=512, d_output=9, d_hidden=1024, q=8, v=8, h=8, N=8,
        # dropout=0.5, pe=True, mask=True, device=device)
        self.GTN_channel = Transformer_chanel(d_model=512, d_input=7500, d_channel=12, d_output=9, d_hidden=1024, q=8,
                                              v=8, h=8, N=8,
                                              dropout=0.5, pe=True, mask=True, device=device)
        self.GTN_time = Transformer_time(d_model=512, d_input=235, d_channel=512, d_output=9, d_hidden=1024, q=8,
                                           v=8, h=8, N=8,
                                           dropout=0.5, pe=True, mask=True, device=device)
        self.gate = torch.nn.Linear(512 * 235 + 512 * 12, 2)
        self.output_linear = torch.nn.Linear(512 * 235 + 512 * 12, 9)
    ...
    def forward(self, x, stage):
        ...
        x_channel = self.GTN_channel(x, stage)
        ...
        # gate
        gate = F.softmax(self.gate(torch.cat([x_time, x_channel], dim=-1)), dim=-1)
        encoding = torch.cat([x_time * gate[:, 0:1], x_channel * gate[:, 1:2]], dim=-1)
...
def DGTN_RE18(**kwargs):
    model = DGTN_RE(BasicBlock1d, [2, 2, 2, 2], **kwargs)
    return model


def DGTN_RE34(**kwargs):
    model = DGTN_RE(BasicBlock1d, [3, 4, 6, 3], **kwargs)
    return model
