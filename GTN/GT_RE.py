...
class Transformer(Module):
    def __init__(self,
        ...
        self.output_linear = torch.nn.Linear(262144, d_output)
    ...
    def forward(self, x, stage):
        ...
        # 输出
        output = self.output_linear(encoding_2)
...
class GT_RE(nn.Module):
    def __init__(self, block, layers, input_channels=12, inplanes=64, num_classes=9, device=''):
        ...
        self.GTN = Transformer(d_model=512, d_input=235, d_channel=512, d_output=9, d_hidden=1024, q=4, v=4, h=4, N=4,
...
def GT_RE18(**kwargs):
    model = GT_RE(BasicBlock1d, [2, 2, 2, 2], **kwargs)
    return model


def Re_GTN34(**kwargs):
    model = GT_RE(BasicBlock1d, [3, 4, 6, 3], **kwargs)
    return model
